...
% legend('SNR= -10dB  NP','SNR=  -5dB  NP','SNR=   0dB  NP',...
%        'SNR=   5dB  NP','SNR=  10dB  NP','SNR=  20dB  NP');
% legend('SNR=  -5dB  IT','SNR=  -4dB  IT','SNR=  -3dB  IT','SNR=  -2dB  IT',...
%        'SNR=  -1dB  IT','SNR=   0dB  IT');

% saveas(h,'I NP(N=1000,SNR=-10,-5,0,5)','fig')
...
%     end
%     T_1_z1=T_1_z1_up./T_1_z1_down;
%     p_1_IT=pfa./((1-T_1_z0)*pfa+T_1_z0);
%     pd_IT=pfa.*T_1_z1./((1-pfa).*T_1_z0+pfa.*T_1_z1);
%     for kx=1:length(x)
%         pd_NP=pd_NP+0.5*erfc((sqrt(2*N)*erfcinv(2* p_1_IT)-SNR*(N-x(kx)))/(sqrt(2*N+4*(N-x(kx))*SNR)))/N*deltax; 
%     end
...
% saveas(IT_NP_ROC,'IT NP ROC','fig')
